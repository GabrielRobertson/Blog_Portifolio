...
?>
<section id="portfolio" class="min-h-screen flex flex-col justify-center py-16 px-4 md:px-0">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-4">
        ...
        <span class="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Projetos & Aplicações</span>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <?php if (!empty($portfolio_projects)): ?>
            <?php foreach ($portfolio_projects as $proj): ?>
            <div class="bg-white rounded-3xl border border-slate-100 p-6 flex flex-col justify-between card-hover shadow-sm group">
                <div>
                    <div class="flex justify-between items-start mb-4">
                        ...
                        <span class="text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full <?php echo ($proj['status'] ?? '') == 'Ativo' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-100 text-slate-600'; ?>">
                            <?php echo htmlspecialchars($proj['status'] ?? 'Concluído'); ?>
                        </span>
                    </div>

                    <h3 class="text-xl font-bold text-slate-800 mb-2 leading-snug group-hover:text-<?php echo $theme_color; ?>-600 transition-colors">
                ...
                <div class="pt-4 border-t border-slate-50">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-[10px] font-mono text-slate-400">Full Stack</span>
